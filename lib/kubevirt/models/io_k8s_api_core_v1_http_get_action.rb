...
module Kubevirt
  ...
  class IoK8sApiCoreV1HTTPGetAction
    ...
    def self.openapi_types
      {
        :'host' => :'String',
        :'http_headers' => :'Array<IoK8sApiCoreV1HTTPHeader>',
        :'path' => :'String',
        :'scheme' => :'String'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kubevirt::IoK8sApiCoreV1HTTPGetAction` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kubevirt::IoK8sApiCoreV1HTTPGetAction`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
