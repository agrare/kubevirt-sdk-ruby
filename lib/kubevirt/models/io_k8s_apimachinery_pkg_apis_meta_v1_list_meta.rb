  ...
  class IoK8sApimachineryPkgApisMetaV1ListMeta
    ...
    def self.openapi_types
      {
        ...
        :'self_link' => :'String',
        :'shard_info' => :'IoK8sApimachineryPkgApisMetaV1ShardInfo'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kubevirt::IoK8sApimachineryPkgApisMetaV1ListMeta` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kubevirt::IoK8sApimachineryPkgApisMetaV1ListMeta`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'shard_info')
        self.shard_info = attributes[:'shard_info']
      end
    end
    ...
    def hash
      [continue, remaining_item_count, resource_version, self_link, shard_info].hash
    end
