...
module Kubevirt
  ...
  class V1LogVerbosity
    ...
    def self.openapi_types
      {
        :'node_verbosity' => :'Hash<String, Integer>',
        :'virt_api' => :'Integer',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kubevirt::V1LogVerbosity` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kubevirt::V1LogVerbosity`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'node_verbosity')
        if (value = attributes[:'node_verbosity']).is_a?(Hash)
          self.node_verbosity = value
        end
      end

      if attributes.key?(:'virt_api')
        self.virt_api = attributes[:'virt_api']
      end

      if attributes.key?(:'virt_controller')
        self.virt_controller = attributes[:'virt_controller']
      end

      if attributes.key?(:'virt_handler')
        self.virt_handler = attributes[:'virt_handler']
      end

      if attributes.key?(:'virt_launcher')
        self.virt_launcher = attributes[:'virt_launcher']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          node_verbosity == o.node_verbosity &&
          virt_api == o.virt_api &&
