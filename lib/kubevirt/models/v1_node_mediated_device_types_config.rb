  ...
  class V1NodeMediatedDeviceTypesConfig
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kubevirt::V1NodeMediatedDeviceTypesConfig` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kubevirt::V1NodeMediatedDeviceTypesConfig`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] node_selector Value to be assigned
    def node_selector=(node_selector)
      if node_selector.nil?
        fail ArgumentError, 'node_selector cannot be nil'
      end
    ...
    def hash
      [mediated_device_types, mediated_devices_types, node_selector].hash
    end
