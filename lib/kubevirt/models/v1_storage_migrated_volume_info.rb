...
module Kubevirt
  ...
  class V1StorageMigratedVolumeInfo
    attr_accessor :destination_pvc_info

    attr_accessor :source_pvc_info
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kubevirt::V1StorageMigratedVolumeInfo` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kubevirt::V1StorageMigratedVolumeInfo`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'destination_pvc_info')
        self.destination_pvc_info = attributes[:'destination_pvc_info']
      end

      if attributes.key?(:'source_pvc_info')
        self.source_pvc_info = attributes[:'source_pvc_info']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          destination_pvc_info == o.destination_pvc_info &&
          source_pvc_info == o.source_pvc_info &&
    ...
    def hash
      [destination_pvc_info, source_pvc_info, volume_name].hash
    end
