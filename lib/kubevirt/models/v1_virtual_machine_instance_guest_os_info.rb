  ...
  class V1VirtualMachineInstanceGuestOSInfo
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kubevirt::V1VirtualMachineInstanceGuestOSInfo` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kubevirt::V1VirtualMachineInstanceGuestOSInfo`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'kernel_release')
        self.kernel_release = attributes[:'kernel_release']
      end
      ...
      if attributes.key?(:'pretty_name')
        self.pretty_name = attributes[:'pretty_name']
      end
    ...
    def hash
      [id, kernel_release, kernel_version, machine, name, pretty_name, version, version_id].hash
    end
