  ...
  class V1beta1StorageSpec
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kubevirt::V1beta1StorageSpec` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kubevirt::V1beta1StorageSpec`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'data_source_ref')
        self.data_source_ref = attributes[:'data_source_ref']
      end
    ...
    # @param [Object] volume_mode Object to be assigned
    def volume_mode=(volume_mode)
      validator = EnumAttributeValidator.new('String', ["Block", "Filesystem", "FromStorageProfile"])
      unless validator.valid?(volume_mode)
        fail ArgumentError, "invalid value for \"volume_mode\", must be one of #{validator.allowable_values}."
      end
    ...
    def hash
      [access_modes, data_source, data_source_ref, resources, selector, storage_class_name, volume_mode, volume_name].hash
    end
